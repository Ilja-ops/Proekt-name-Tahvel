...
# aadressiväli (välja sees)
    self.entry_address = ctk.CTkEntry(frame, placeholder_text="Aadress")
    self.entry_address.pack(pady=5, fill="x")
...
# tsükkel loob tabelis vasakul asuva allkirjade loendi, milles on väljade nimed
       for i in range(len(lists)):
        ctk.CTkLabel(table,text=lists[i],width=200,anchor="w").grid(row=i, column=0, padx=20, pady=8, sticky="w")
...
# loob tabeli paremale serva allkirjade loendi, mis sisaldab kasutaja andmeid
        ctk.CTkLabel(table,text=andmed[i].strip(),anchor="w").grid(row=i, column=1, padx=20, pady=8, sticky="w")
